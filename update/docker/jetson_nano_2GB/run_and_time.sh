...
docker build --memory="2g" --memory-swap="5g" --memory-swappiness=100 -t $image -f Dockerfile.$device .
...
opts="--user_conf ./mlperf.conf --profile $profile $common_opt --model $model_path \
    --dataset-path $DATA_DIR --output $OUTPUT_DIR $extra_args $EXTRA_OPS $@"
